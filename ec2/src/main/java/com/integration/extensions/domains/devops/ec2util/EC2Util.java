...
public class EC2Util {
    ...
    public static AmazonEC2 getEC2(Map<String, Object> connectionAttributes) throws ExtensionException {
    ...
    public static com.integration.extensions.domains.devops.Instance getInstanceById(AmazonEC2 ec2, String instanceId) {
        return describeInstances(ec2).stream().filter(instance -> instance.instanceId.toLowerCase().contains(instanceId.toLowerCase())).findFirst().orElse(null);
    }

    public static List<com.integration.extensions.domains.devops.Instance> getInstances(AmazonEC2 ec2) {
           return describeInstances(ec2);
       }

    public static List<com.integration.extensions.domains.devops.Instance> getInstancesById(AmazonEC2 ec2, String instanceId) {
        return describeInstances(ec2).stream().filter(instance -> instance.instanceId.toLowerCase().contains(instanceId.toLowerCase())).collect(Collectors.toList());
    }

    public static List<com.integration.extensions.domains.devops.Instance> getInstancesByName(AmazonEC2 ec2, String name) {
        return describeInstances(ec2).stream().filter(instance -> instance.keyName.toLowerCase().contains(name.toLowerCase())).collect(Collectors.toList());
    }
    ...
    public static List<com.integration.extensions.domains.devops.InstanceType> getInstanceTypes(AmazonEC2 ec2, List<String> instanceType, long size) {
        String nextToken = null;
    ...
    public static List<String> getInstanceTypesByName(String name) {
        return getInstanceTypes().stream().filter(instanceType -> instanceType.toLowerCase().contains(name)).collect(Collectors.toList());
    }

    public static List<String> getInstanceTypes() {
        return Arrays.stream(com.amazonaws.services.ec2.model.InstanceType.values()).map(com.amazonaws.services.ec2.model.InstanceType::toString).collect(Collectors.toList());
    }
