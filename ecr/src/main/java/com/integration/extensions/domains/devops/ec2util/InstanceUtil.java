...
public class InstanceUtil {
    ...
    public static Instance getInstance(Reservation reservation, com.amazonaws.services.ec2.model.Instance inst) {
        ...
        instance.numberOfCores =
                inst.getCpuOptions().getCoreCount() != null ? inst.getCpuOptions().getCoreCount().longValue() : null;
        instance.threadsPerCore =
                inst.getCpuOptions().getThreadsPerCore() != null ? inst.getCpuOptions().getThreadsPerCore().longValue()
                        : null;
        instance.capacityReservationPreference =
                inst.getCapacityReservationSpecification().getCapacityReservationPreference();
        return instance;
    }
    ...
    public static InstanceType getInstanceType(InstanceTypeInfo type) {
        ...
        instanceType.defaultNumberOfVCpus =
                type.getVCpuInfo().getDefaultVCpus() != null ? type.getVCpuInfo().getDefaultVCpus().longValue() : null;
        instanceType.defaultNumberOfCores =
                type.getVCpuInfo().getDefaultCores() != null ? Long.valueOf(type.getVCpuInfo().getDefaultCores())
                        : null;
        instanceType.defaultNumberOfThreadsPerCore =
                type.getVCpuInfo().getDefaultThreadsPerCore() != null ? type.getVCpuInfo().getDefaultThreadsPerCore()
                        .longValue() : null;
